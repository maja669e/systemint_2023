...
#parsed_content.find('div', { "id": "bodyContent" }).find_all('a', href=re.compile("^(/wiki/)((?!:).)*$"))
...
def get_internal_wiki_links_tags(parsed_page):
...
root_internal_links = get_internal_wiki_links_tags(parsed_root_page)
...
while True:
    for _ in range(4):
        ...
        for link_to_visit in to_visit_queue:
            ...
            internal_links = get_internal_wiki_links_tags(parsed_page)
